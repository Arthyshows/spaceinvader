...
// ########################
	...
	// 1.6
	int CLAVIER_SOURIS_AUTO = 1;
	#define MAX_TOUCHES 500
	#define TAILLE_CLIC_TAMPON 10
	
	struct EtatClavierSouris{
		BOOL toucheEnfoncee[MAX_TOUCHES];
		int nbToucheRelachee[MAX_TOUCHES];
		POINT clicTampon[TAILLE_CLIC_TAMPON];
		int idPremierClic;
		int nbClicATraiter;
	} ETAT_CLAVIER_SOURIS;
...
POINT get_arrow()
	...
	POINT dep;
	dep.x = dep.y = 0;
	if(!CLAVIER_SOURIS_AUTO){
		if(ETAT_CLAVIER_SOURIS.toucheEnfoncee[SDLK_LEFT]) {dep.x -= MINDEP;}
		if(ETAT_CLAVIER_SOURIS.toucheEnfoncee[SDLK_RIGHT]) {dep.x += MINDEP;}
		if(ETAT_CLAVIER_SOURIS.toucheEnfoncee[SDLK_UP]) {dep.y += MINDEP;}
		if(ETAT_CLAVIER_SOURIS.toucheEnfoncee[SDLK_DOWN]) {dep.y -= MINDEP;}
		return dep;
	}
...
POINT wait_clic()
	...
	if(!CLAVIER_SOURIS_AUTO){
		POINT clic;
		while(get_clic(&clic)){}  // vide le tampon
		while(!get_clic(&clic)){
...
void clavier_souris_auto_on(){
	CLAVIER_SOURIS_AUTO = 1;
}

void clavier_souris_auto_off(){
	int k=0;
	CLAVIER_SOURIS_AUTO = 0;
	for(k=0;k<MAX_TOUCHES;k++){
		ETAT_CLAVIER_SOURIS.toucheEnfoncee[k] = FALSE;
		ETAT_CLAVIER_SOURIS.nbToucheRelachee[k]=0;
	}
	ETAT_CLAVIER_SOURIS.idPremierClic = 0;
	ETAT_CLAVIER_SOURIS.nbClicATraiter = 0;
}

BOOL get_clic(POINT *clic){
	if(ETAT_CLAVIER_SOURIS.nbClicATraiter>0){
		(*clic) = ETAT_CLAVIER_SOURIS.clicTampon[ETAT_CLAVIER_SOURIS.idPremierClic];
		ETAT_CLAVIER_SOURIS.nbClicATraiter -=1;
		ETAT_CLAVIER_SOURIS.idPremierClic = (ETAT_CLAVIER_SOURIS.idPremierClic+1)%TAILLE_CLIC_TAMPON;
		return TRUE;
	}
	return FALSE;
}

void update_clavier_souris(){
	SDL_Event event;
	int k;
	POINT P;
	for(k=0;k<MAX_TOUCHES;k++){
	...
	while (SDL_PollEvent(&event) )
		...
		if (event.type == SDL_KEYDOWN)
			{
				ETAT_CLAVIER_SOURIS.toucheEnfoncee[event.key.keysym.sym] = TRUE;
			}
			
		/* Si l'utilisateur a relaché sur une touche */
		if (event.type == SDL_KEYUP)
			{
				ETAT_CLAVIER_SOURIS.toucheEnfoncee[event.key.keysym.sym] = FALSE;
				ETAT_CLAVIER_SOURIS.nbToucheRelachee[event.key.keysym.sym] += 1;
			}
		
		/* Si l'utilisateur a cliqué gauche */
		if ((event.type == SDL_MOUSEBUTTONDOWN) && (event.button.button == SDL_BUTTON_LEFT))
		{
			
			P.x = event.button.x;
			P.y = HEIGHT-event.button.y;
			ETAT_CLAVIER_SOURIS.clicTampon[ (ETAT_CLAVIER_SOURIS.idPremierClic+ETAT_CLAVIER_SOURIS.nbClicATraiter)%TAILLE_CLIC_TAMPON ] = P;
			ETAT_CLAVIER_SOURIS.nbClicATraiter += 1;
			if(ETAT_CLAVIER_SOURIS.nbClicATraiter>TAILLE_CLIC_TAMPON){
				ETAT_CLAVIER_SOURIS.nbClicATraiter-=1;
				ETAT_CLAVIER_SOURIS.idPremierClic = (ETAT_CLAVIER_SOURIS.idPremierClic+1)%TAILLE_CLIC_TAMPON;
			}
		}
		...
		if (event.type == SDL_MOUSEMOTION)
			{
			___MOUSE_POSITION.x = event.motion.x;
			___MOUSE_POSITION.y = HEIGHT-event.motion.y;
			}
...
BOOL touche_espace(){
	return ETAT_CLAVIER_SOURIS.toucheEnfoncee[SDLK_SPACE];
}
